...
function firstdoor(success, timeremaining)
    if success then
        TriggerEvent("mrp-doors:changeLock-status", 160, false)
        ...
        ExecuteCommand('e c')
		TriggerEvent('mrp:alert:powerplant')
    end
end

function seconddoor(success, timeremaining)
    if success then
        TriggerEvent("mrp-doors:changeLock-status", 161, false)
        TriggerEvent('mhacking:hide')
		TriggerEvent("DoLongHudText","Success!")
        ExecuteCommand('e c')
    else
        ...
        if distance < 1.5 then
            ...
            if IsControlJustReleased(0,38) and distance < 1.5 then
                 if exports["mrp-inventory"]:hasEnoughOfItem("drill",1,false) then
					...
					TriggerEvent('mrp:alert:paleto')
                    local finished = exports["mrp-taskbar"]:taskBar(50000,"Fidgeting with bankbox")
