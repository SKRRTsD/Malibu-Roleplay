...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 5.0 then
            ...
            if IsControlJustReleased(0,38) and distance < 5.0 then
                if exports["mrp-inventory"]:hasEnoughOfItem("Gruppe6Card222",1,false ) then
                if exports["mrp-inventory"]:hasEnoughOfItem("laptop2",1,false ) then
    ...
    while true do 
        ...
        if distance < 1.5 then
            ...
            if IsControlJustReleased(0,38) and distance < 1.5 then 
                 if exports["mrp-inventory"]:hasEnoughOfItem("Gruppe6Card2",1,false) then
                    if exports['mrp-inventory']:hasEnoughOfItem('laptop3', 1) then
                    StartVaultKid()
                    local vault = exports["mrp-taskbar"]:taskBar(9000,"Hooking up Laptop!")
                    if vault == 100 then
					...
					TriggerEvent('mrp:alert:bigbank2')
                    Citizen.Wait(3600000)
                            end
                        end
                    end
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 1.5 then
            ...
            Wait(1)
            if IsControlJustReleased(0,38) and distance < 1.5 then 
                if exports["mrp-inventory"]:hasEnoughOfItem("thermite",5,false) then
					-- TriggerEvent('mrp:alert:bigbank2')
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if distance < 1.5 then
            ...
            Wait(1)
            if IsControlJustReleased(0,38) and distance < 1.5 then
                 if exports["mrp-inventory"]:hasEnoughOfItem("drill",1,false) then
                    TriggerEvent("animation:PlayAnimation","welding")
                    local finished = exports["mrp-taskbar"]:taskBar(50000,"Drilling Into Bankbox")
...
function upstairshacking(success, timeremaining)
    if success then
        TriggerEvent("mrp-doors:changeLock-status", 157, false)
...
AddEventHandler('av_vault2:bomba', function()
	...
	RequestAnimDict('anim@heists@ornate_bank@thermal_charge')
	 HasAnimDictLoaded('anim@heists@ornate_bank@thermal_charge') 
			...
			seguridad = false
			FreezeEntityPosition(PlayerPedId(),false)
			TriggerEvent("mrp-doors:changeLock-status", 158, false)
...
AddEventHandler('av_vault3:bomba', function()
	...
	RequestAnimDict('anim@heists@ornate_bank@thermal_charge')
	 HasAnimDictLoaded('anim@heists@ornate_bank@thermal_charge') 
			...
			seguridad = false
			FreezeEntityPosition(PlayerPedId(),false)
			TriggerEvent("mrp-doors:changeLock-status", 159, false)
			TriggerServerEvent('av_vangelico:gas')
end)
